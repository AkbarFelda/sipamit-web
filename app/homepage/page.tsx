  ...
  return (
    <MobileContainer className="bg-linear-to-b from-blue-top to-blue-bottom p-6 flex flex-col min-h-screen">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="text-lg font-bold text-black">{user}</h1>
          </div>
        </div>
        <button
      ...
      <div className="bg-white/30 backdrop-blur-md p-4 rounded-2xl mb-8 border border-white/20">
        <p className="text-xs font-bold text-blue-800 uppercase tracking-wider mb-1">Informasi</p>
      ...
      <div className="grid grid-cols-2 gap-4 mb-10">
        ...
        />
        <MenuCard icon={User} label={"SPK Pelayanan Lain"} count={stats.pelayananLain} />
        <MenuCard icon={User} label={"SPK Pengaduan"} count={stats.pengaduan} />
        <MenuCard icon={User} label={"SPK Penyegelan"} count={stats.penyegelan} />
        <MenuCard icon={User} label={"SPK Pergantian Meter"} count={stats.pergantianMeter} />
        <MenuCard icon={User} label={"SPK Pemutusan"} count={stats.pemutusan} />
      </div>
    </MobileContainer>
  );
}