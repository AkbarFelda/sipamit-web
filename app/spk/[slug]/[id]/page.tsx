    ...
    return (
      <MobileContainer className="bg-gray-50 flex flex-col items-center justify-center min-h-screen text-black">
        <div className="text-center p-6">
          ...
          <button onClick={() => router.back()} className="bg-blue-600 text-white px-6 py-2 rounded-xl shadow-lg">
            Kembali
          </button>
        </div>
      </MobileContainer>
  ...
  return (
    <MobileContainer className="bg-gray-50 flex flex-col min-h-screen pb-32 text-black">
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="bg-white p-6 rounded-4xl shadow-sm border border-gray-100 flex flex-col items-center text-center">
          <div className="w-20 h-20 bg-blue-600 rounded-full flex items-center justify-center text-white text-2xl font-bold mb-3 shadow-lg ring-4 ring-blue-50">
            {(detail?.nama || detail?.namajenis || "U").substring(0, 2).toUpperCase()}
          </div>
          <h2 className="text-xl font-black text-gray-800 capitalize leading-tight">{detail?.nama || detail?.namajenis}</h2>

          <div className="flex flex-wrap justify-center gap-2 mt-3">
            <Badge label={isDone ? "SELESAI" : "TODO / PROSES"} color={isDone ? "green" : "orange"} />
            {detail?.no_aduan && <Badge label={`ADUAN: ${detail.no_aduan}`} color="orange" />}
            {detail?.no_regis && <Badge label={`REG: ${detail.no_regis}`} color="green" />}
            {detail?.no_pelanggan && <Badge label={`PEL: ${detail.no_pelanggan}`} color="blue" />}
            {detail?.no_rab && <Badge label={`RAB: ${detail.no_rab}`} color="purple" />}
          </div>
        </div>

        {/* SECTION 2: BUKTI PENYELESAIAN (Satu blok saja) */}
        {isDone && (
          <div className="bg-green-50/20 p-5 rounded-4xl shadow-sm border border-green-200 space-y-4">
            ...
            {/* Info Dinamis: Merek Meter untuk Segel/Buka Segel */}
            <div className="p-3 bg-white rounded-2xl border border-green-100 flex items-center justify-between shadow-sm">
              <div className="flex items-center gap-2">
                ...
                <p className="text-[10px] font-black text-gray-400 uppercase">Merek Meter Terpasang</p>
              </div>
              <p className="font-black text-green-700">{detail?.merek_meter || "-"}</p>
            </div>

            <div className="grid grid-cols-2 gap-4">
              {/* Foto Bukti (Dinamis Buka vs Proses) */}
              <div className="space-y-2">
                ...
                <div
                  ...
                  onClick={() => handlePreview(slug === "buka-segel" ? detail?.url_foto_buka : detail?.url_foto_proses)}
                >
                  {(slug === "buka-segel" ? detail?.url_foto_buka : detail?.url_foto_proses) ? (
                    <Image
                      src={getImageUrl(slug === "buka-segel" ? detail?.url_foto_buka : detail?.url_foto_proses)}
                      alt="Bukti Kerja"
                      fill
              ...
              {/* Foto Tanda Tangan (Dinamis Buka vs Proses) */}
              <div className="space-y-2">
                <p className="text-[10px] font-black text-gray-400 uppercase flex items-center gap-1 italic">
                ...
                <div
                  ...
                  onClick={() => handlePreview(slug === "buka-segel" ? detail?.url_foto_ttd_buka : detail?.url_foto_ttd_proses)}
                >
                  {(slug === "buka-segel" ? detail?.url_foto_ttd_buka : detail?.url_foto_ttd_proses) ? (
                    <Image
                      src={getImageUrl(slug === "buka-segel" ? detail?.url_foto_ttd_buka : detail?.url_foto_ttd_proses)}
                      alt="TTD"
                      fill
                    ...
                    <div className="flex items-center justify-center h-full text-gray-300"><Signature size={24} /></div>
                  )}
                </div>
              </div>
            </div>
        ...
        {slug === "pengaduan" && detail?.url_foto_aduan && (
          <div className="bg-white p-5 rounded-4xl shadow-sm border border-gray-100">
            ...
            >
              <Image src={getImageUrl(detail.url_foto_aduan)} alt="Lampiran" fill className="object-cover" unoptimized />
        ...
        <div className="bg-white p-5 rounded-4xl shadow-sm border border-gray-100 space-y-5">
          <div className="flex items-start gap-4">
            <div className="bg-green-50 p-3 rounded-2xl text-green-600"><MapPin size={22} /></div>
            <div className="flex-1">
              ...
              <button
                ...
                onClick={() => window.open(`https://www.google.com/maps/search/?api=1&query=${detail?.latitude},${detail?.longitude}`, "_blank")}
                className="mt-2 text-blue-600 font-black text-[10px] uppercase underline"
              >
                Navigasi Maps
              </button>
            </div>
          ...
          <div className="flex items-center gap-4 pt-4 border-t border-gray-100">
            <div className="bg-blue-50 p-3 rounded-2xl text-blue-600"><Phone size={22} /></div>
            <div className="flex-1">
        ...
        <div className="bg-white p-5 rounded-4xl shadow-sm border border-gray-100 space-y-4">
          <h3 className="font-bold text-gray-800 flex items-center gap-2 mb-2 text-sm"><ClipboardList size={18} className="text-blue-600" /> Detail Teknis</h3>
        ...
        <div className="bg-white p-5 rounded-4xl shadow-sm border border-gray-100">
          <h3 className="font-bold text-gray-800 flex items-center gap-2 mb-4 text-sm"><CreditCard size={18} className="text-blue-600" /> Rincian Biaya</h3>
          <div className="space-y-3">
            ...
            <div className="pt-4 border-t-2 border-dashed border-gray-100 mt-4 flex justify-between items-center">
              <div>
                ...
                <p className="text-xl font-black text-blue-600">{formatRupiah(detail?.total || detail?.biaya_bukasegel)}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {isImgOpen && previewImg && (
        <div className="fixed inset-0 z-100 bg-black/95 flex items-center justify-center p-4" onClick={() => setIsImgOpen(false)}>
          <button className="absolute top-10 right-6 text-white bg-white/10 p-3 rounded-full"><X size={30} /></button>
          <div className="relative w-full h-[70vh]">
            <Image src={previewImg} alt="Preview" fill className="object-contain" unoptimized />
          </div>
        </div>
      )}
    </MobileContainer>
